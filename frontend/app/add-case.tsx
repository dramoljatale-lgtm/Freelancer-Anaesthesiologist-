...
import { addCase, getHospitals, getSurgeons, addHospital, addSurgeon, NamedItem } from '../utils/storage';

const GENDERS = [{ label: 'Male', value: 'Male' }, { label: 'Female', value: 'Female' }, { label: 'Other', value: 'Other' }];
const ANAESTHESIA_TYPES = [{ label: 'General', value: 'General' }, { label: 'Regional', value: 'Regional' }, { label: 'Local', value: 'Local' }, { label: 'Sedation', value: 'Sedation' }, { label: 'Combined', value: 'Combined' }];
const PAYMENT_MODES = [{ label: 'Cash', value: 'Cash' }, { label: 'UPI', value: 'UPI' }, { label: 'Cheque', value: 'Cheque' }, { label: 'Bank Transfer', value: 'Bank Transfer' }];

const getDefaultDate = () => { const d = new Date(); return `${d.getDate().toString().padStart(2, '0')}/${(d.getMonth() + 1).toString().padStart(2, '0')}/${d.getFullYear()}`; };
...
export default function AddCase() {
  ...
  useEffect(() => {
    ...
    getSurgeons().then(setSurgeons);
  }, []);

  const handleAddHospital = async (name: string) => {
    const item = await addHospital(name);
    setHospitals(await getHospitals());
  };
  const handleAddSurgeon = async (name: string) => {
    const item = await addSurgeon(name);
    setSurgeons(await getSurgeons());
  };

  const handleUseAmount = (fee: number, details: ISARVGDetailsType) => { setFees(fee.toString()); setIsaDetails(details); setCalcVisible(false); };
  ...
  const handleSave = async (paymentStatus: string) => {
    ...
    setSaving(true); setSavingStatus(paymentStatus);
    try {
      await addCase({
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      <View style={s.header}>
        <TouchableOpacity testID="back-btn" onPress={() => router.back()} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}><Ionicons name="chevron-back" size={24} color="#1A201C" /></TouchableOpacity>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.row}>
            <View style={{ flex: 1 }}><Text style={s.label}>Age</Text><TextInput testID="age-input" style={s.input} value={age} onChangeText={setAge} placeholder="Age" keyboardType="numeric" placeholderTextColor="#9CA3AF" /></View>
            <View style={{ flex: 1.4 }}><Text style={s.label}>Gender</Text><CustomPicker testID="gender-select" options={GENDERS} selectedValue={gender} onValueChange={setGender} placeholder="Gender" /></View>
          </View>
          ...
          <AddablePicker testID="surgeon-select" items={surgeons} selectedValue={surgeonName} onValueChange={setSurgeonName} onAddNew={handleAddSurgeon} placeholder="Surgeon" />
          <Text style={s.label}>Hospital</Text>
          <AddablePicker testID="hospital-select" items={hospitals} selectedValue={hospital} onValueChange={setHospital} onAddNew={handleAddHospital} placeholder="Hospital" />
          <View style={s.row}>
            <View style={{ flex: 1 }}><Text style={s.label}>Date</Text><TextInput testID="date-input" style={s.input} value={date} onChangeText={setDate} placeholder="DD/MM/YYYY" placeholderTextColor="#9CA3AF" /></View>
            <View style={{ flex: 1.4 }}><Text style={s.label}>Anaesthesia Type</Text><CustomPicker testID="anaesthesia-type-select" options={ANAESTHESIA_TYPES} selectedValue={anaesthesiaType} onValueChange={setAnaesthesiaType} placeholder="Type" /></View>
          </View>
          ...
          <TouchableOpacity testID="calculate-isa-fee-btn" style={s.calcBtn} onPress={() => setCalcVisible(true)} activeOpacity={0.7}>
            ...
            <View style={{ flex: 1 }}><Text style={s.calcBtnTitle}>Calculate Standard Fee</Text><Text style={s.calcBtnSub}>ISA-RVG based guidelines</Text></View>
            <Ionicons name="chevron-forward" size={18} color="#4A7C59" />
          </TouchableOpacity>
          {isaDetails && (<View style={s.isaBadge}><View style={s.isaBadgeTop}><Ionicons name="checkmark-circle" size={18} color="#4A7C59" /><Text style={s.isaBadgeTitle}>Standard Fee Applied</Text></View><Text style={s.isaBadgeDetail}>{isaDetails.total_units} units × ₹{formatINR(isaDetails.rate_per_unit)} = ₹{formatINR(isaDetails.base_fee)}{isaDetails.emergency ? ' + 30% emergency' : ''}</Text><Text style={s.isaBadgeFee}>Final: ₹{formatINR(isaDetails.final_fee)}</Text></View>)}
...
const s = StyleSheet.create({
  ...
  scroll: { flex: 1 }, scrollContent: { paddingHorizontal: 20, paddingTop: 4 },
  ...
  calcBtnTitle: { fontSize: 14, fontWeight: '700', color: '#4A7C59' }, calcBtnSub: { fontSize: 12, color: '#6B7280', marginTop: 2 },
